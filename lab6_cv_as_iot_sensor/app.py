...
DEFAULT_CAMERA_SOURCE = "http://172.16.10.249:8080/video"
...
def laptop_camera_backends() -> List[int]:
    """Return preferred OpenCV backends for a built-in laptop webcam."""
...
def open_capture(source: str = DEFAULT_CAMERA_SOURCE) -> Optional[cv2.VideoCapture]:
    parsed_source = parse_camera_source(source)
    is_laptop_camera = isinstance(parsed_source, int)
    backends = laptop_camera_backends() if is_laptop_camera else [cv2.CAP_ANY]
...
def read_one_frame(source: str = DEFAULT_CAMERA_SOURCE) -> Tuple[np.ndarray, str]:
...
def camera_status(source: str = Query(DEFAULT_CAMERA_SOURCE, description="0 for laptop camera, or IP camera/video URL")) -> Dict[str, Any]:
    cap = open_capture(source)
    if cap is None:
        return {
            "source": source,
            "available": False,
            "message": "Không mở được nguồn camera. Với IP Webcam, kiểm tra điện thoại và máy tính cùng Wi-Fi, app IP Webcam đang Start server, và URL có dạng http://IP:8080/video.",
        }
    ok, frame = cap.read()
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) or (int(frame.shape[1]) if ok and frame is not None else 0)
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) or (int(frame.shape[0]) if ok and frame is not None else 0)
    cap.release()
    return {
        "source": source,
        ...
        "width": width,
        "height": height,
        "message": "Camera laptop đã sẵn sàng." if ok and frame is not None else "Camera mở được nhưng chưa đọc được frame.",
    }
...
@app.get("/snapshot")
def snapshot(source: str = Query(DEFAULT_CAMERA_SOURCE, description="0 for laptop camera, or IP camera/video URL")) -> Dict[str, Any]:
...
def record_video(source: str = Query(DEFAULT_CAMERA_SOURCE), seconds: int = Query(5, ge=1, le=30)) -> Dict[str, Any]:
    return record_short_video(source, seconds=seconds)


@app.get("/motion-capture")
def motion_capture_endpoint(source: str = Query(DEFAULT_CAMERA_SOURCE), seconds: int = Query(8, ge=1, le=30), threshold: int = Query(25, ge=1, le=255), min_area: int = Query(800, ge=10, le=50000)) -> Dict[str, Any]:
    return motion_capture(source, seconds=seconds, threshold=threshold, min_area=min_area)


@app.get("/video_feed")
def video_feed(source: str = Query(DEFAULT_CAMERA_SOURCE)) -> StreamingResponse:
